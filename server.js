...
const limiterChat     = createRateLimiter(15,   60_000, 2 * 60_000);  // 15 msgs/min, block 2 min
...
// ── API: AI Chat (Groq — free) ───────────────────────────────────────────────
const TRISEND_SYSTEM_MSG = {
  role: 'system',
  content: `You are Trisend AI, the helpful assistant inside Trisend — a link management SaaS. Be friendly, concise, and practical. Reply in plain HTML: use <br> for line breaks and <strong> for bold text. No markdown. Keep replies under 4 short paragraphs.

TRISEND FEATURES:
QR Generator: 8 types (URL, WiFi, vCard, Email, Phone, SMS, App Download, Multi-link). Free: up to 10 codes. Premium adds custom colors, shapes, frames, logo embed, and Dynamic QR (edit the destination anytime without reprinting).
Short Links: paste any URL, get a trackable short link. Every click is recorded automatically. Premium adds custom alias, UTM Campaign Builder, link scheduling (set a go-live and expiry time), max-click limits, password protection, geo-redirect (send visitors to different URLs by country). Free: up to 20 links.
Analytics: real-time dashboard — QR scans + link clicks, 7-day click trend chart, per-link detail screen. Premium adds device breakdown, browser breakdown, top countries, CSV export.
Link-in-Bio: one personal page (trisend.onrender.com/u/username) listing all your links — Premium only.
Plans: Free (10 QRs, 20 links, basic analytics). Premium: NGN 2,000/month Nigeria, ~$2/month international. Exactly 30 days full access per payment. No auto-renewal ever.
Support: trisendmailer@gmail.com or Settings > Report a Bug.
If asked about specific account data, tell the user to check the relevant dashboard section — you cannot access live account data.`
};

app.post('/api/chat', limiterChat, express.json({ limit: '20kb' }), async (req, res) => {
  if (!GROQ_API_KEY) {
    return res.status(503).json({ error: 'AI not configured. Add GROQ_API_KEY env var on Render.' });
  }
  ...
  // Sanitise + cap history at 10 messages
  ...
  if (!clean.length || clean[0].role !== 'user') {
    return res.status(400).json({ error: 'Must start with a user message.' });
  }

  try {
    // Groq uses OpenAI format — system prompt goes as first message
    const data = await groqRequest([TRISEND_SYSTEM_MSG, ...clean]);
